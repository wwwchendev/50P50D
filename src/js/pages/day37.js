...
import '../../stylesheet/page/_day37.scss';
...
const day37 = () => {
  ...
  const mainTypes = Object.keys(types);
  ...
  const createPokemonCard = (pokemon) => {
    ...
    const pokeTypes = pokemon.types.map((type) => type.type.name); // ["electric","flying"]
    // 在主要屬性 mainTypes 陣列中尋找第一個與 Pokémon 的屬性清單 pokeTypes 中的元素相符的主要 Pokémon 類型。
    // 它透過檢查 pokeTypes 中的元素是否存在於 mainTypes 中來找到第一個匹配的屬性
    // Array.indexOf() 用於在陣列中尋找特定元素的方法。如果元素存在於數組中，它會傳回該元素的索引，否則傳回 -1。
    const type = mainTypes.find((_type) => pokeTypes.indexOf(_type) > -1);
